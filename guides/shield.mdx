...
| ----------------------- | ------- | --------------------------------------------------- | ---------------------------- |
| `email`                 | string  | Customer email address                              | `john.doe@example.com`       |
| `device_fingerprint`    | string  | Unique fingerprint of the device                    | `a3f7b2c1d9e8f0a4b5c6d7e8`   |
| `card_fingerprint`      | string  | Unique fingerprint of the Card                      | `4f8a2c1b7d3e9f0a5b6c7d8e`   |
| `email_domain`          | string  | Domain part of the email (after @)                  | `example.com`                |
| `ip`                    | string  | Customer IP address                                 | `192.168.1.1`                |
| `ip_country`            | string  | Customer IP address                                 | `US`                         |
| `country`               | string  | Two-letter country code from billing address        | `US`                         |
| `card_bin`              | string  | First 6 digits of the card number                   | 411111                       |
| `card_country`          | string  | Card issuing country                                | `US`                         |
| `card_funding`          | string  | `credit`, `debit`, or `prepaid`                     | `credit`                     |
| `amount`                | integer | Payment amount in cents                             | `2999`                       |
| `currency`              | string  | Three-letter currency code                          | `USD`                        |
| `risk_score`            | integer | Pandabase risk score (0–100)                        | `42`                         |
| `risk_level`            | enum    | Can be one of `LOW`, `MEDIUM` and `HIGH`            | `MEDIUM`                     |
| `is_new_customer`       | boolean | True if the customer has no prior completed orders  | `true`                       |
| `customer_order_count`  | integer | Number of completed orders by this customer         | `3`                          |
| `customer_has_disputed` | boolean | Has this customer previously disputed               | `false`                      |
| `ip_has_disputed`       | boolean | Has this IP previously disputed                     | `false`                      |
| `device_has_disputed`   | boolean | Has this device previously disputed                 | `false`                      |
| `isp`                   | integer | The customer's Internet Service Provider (ISP) name | `Comcast Cable`              |
| `browser`               | integer | The customer's browser name and version             | `Chrome 124.0`               |
| `metadata.*`            | string  | Checkout metadata values (e.g. `metadata.source`)   | `metadata.source=google_ads` |
...
<Warning>
  Be careful with ALLOW rules at low priority numbers. An overly broad ALLOW
  rule can bypass all your other protections.
</Warning>
...
<Note>
  You'll receive an email and a `PAYMENT_REVIEW` webhook event when a payment is
  held for review.
</Note>
