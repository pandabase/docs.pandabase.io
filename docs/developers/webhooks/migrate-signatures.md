...
Your endpoint is still on **V1**. A Standard Webhooks SDK assumes the V2 wire
format, so on a V1 delivery it fails on all three counts: the signature is
bare hex (not `v1,<base64>`), the signed string is `${timestamp}.${rawBody}`
rather than `${Webhook-Id}.${timestamp}.${rawBody}`, and `Webhook-Timestamp`
is in **milliseconds** — which a seconds-based verifier reads as a far-future
time and rejects as outside the replay window. The error often *looks* like
the sender sent a bad timestamp; it didn't — the verifier and the endpoint
are on different modes.

**Fix:** switch the endpoint to V2 (then the SDK works), or verify with
[`verifyV1`](#v1-replay-protection) until you do. Confirm the mode with
`signatureVersion`.
...
You're almost certainly hashing a re-serialized body. Hash the **raw** bytes
