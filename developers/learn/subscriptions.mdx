...
# pause billing
POST /v2/core/stores/{storeId}/subscriptions/{subscriptionId}/pause

# resume billing
POST /v2/core/stores/{storeId}/subscriptions/{subscriptionId}/resume
```
...
If you need to charge per request, per token, per seat, or any other metric where the bill is not known in advance, configure the product with `pricingModel: USAGE_BASED` and attach meters. The lifecycle, retries, invoices, and webhook events described on this page apply unchanged. See [Usage-based billing](/developers/learn/usage-based-billing) for the meter model, event ingestion, and renewal math.
