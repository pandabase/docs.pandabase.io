...
- Fixed platform fee from 5% to 6% + 20¢ per transaction.
- Fixed soft-deleted products still appearing in checkout item resolution.
...
**Key changes from V1:**
- New response format: `{ ok: true, data }` / `{ ok: false, error }` (replaces raw response objects)
- New pagination format: `{ items, pagination: { page, limit, total, totalPages } }` (replaces `meta` object)
- TypeBox schema validation (replaces Yup)
- Magic link authentication (no passwords)
- All monetary values in cents (integers, no floats)

### New Modules

- **Checkouts**: Redis-backed sessions with custom fields (text, numeric, dropdown), coupon/tax support, and pay-what-you-want pricing
...
- Distributed locking (Redlock) for safe concurrent payout processing
...
Merchants can now onboard from: US, GB, AT, BE, CY, EE, FI, FR, DE, GR, IE, IT, LV, LT, LU, MT, NL, PT, SK, SI, ES, BG, HR, DK, HU, RO, SE, NO, CH, CZ, PL.
