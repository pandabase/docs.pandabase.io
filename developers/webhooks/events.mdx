...
The order status will be `PROCESSING` (if fulfillment is in progress) or `COMPLETED` (if already fulfilled). The `items` array contains all purchased products, and `customFields` includes any data the customer submitted at checkout.
...
Fired when a charge is refunded, whether initiated through the Pandabase dashboard or detected from an external Stripe refund. The order status is set to `REFUNDED`.
