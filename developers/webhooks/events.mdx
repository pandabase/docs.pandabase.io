...
Subscription events follow the same payload structure as payment events. They are fired alongside (not instead of) payment events — for example, a successful renewal triggers both `PAYMENT_COMPLETED` and `SUBSCRIPTION_RENEWED`.
