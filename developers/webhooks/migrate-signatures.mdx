...
description: "Upgrade your endpoint to Standard Webhooks v2, or migrate off the legacy X-Pandabase-* headers."
---

Each webhook endpoint runs in one of two signature modes:

- **V2** — [Standard Webhooks](https://www.standardwebhooks.com) v1. SDKs verify it for free. Default for endpoints created after 2026-05-15.
- **V1** — Hex `Webhook-Signature` over `${timestamp}.${rawBody}` (ms), plus legacy `X-Pandabase-*` headers. Default for older endpoints.

V1 keeps working — switch when you're ready.

## Which mode am I on?

`GET /v2/stores/:storeId/webhooks/:webhookId` returns `signatureVersion`. Same field shown on the dashboard's webhook page.

## V1 → V2

1. **Deploy your V2 verifier first.** Snippets in the [overview](/developers/webhooks/overview#verification). Easiest: drop in a [Standard Webhooks SDK](https://www.standardwebhooks.com/#libraries).
2. **Flip the mode:**

   ```bash
   curl -X PATCH https://api.pandabase.io/v2/stores/{storeId}/webhooks/{webhookId} \
     -H "Authorization: Bearer $SESSION_TOKEN" \
     -H "Content-Type: application/json" \
     -d '{ "signatureVersion": "V2" }'
   ```

3. **Test:** `POST /v2/stores/{storeId}/webhooks/{webhookId}/test`. The test mirrors real deliveries.
4. **Roll back** with `"signatureVersion": "V1"` if needed.
...
Still verifying `X-Pandabase-Signature`? Move to `Webhook-Signature` (the V1 endpoint already sends it). You get replay protection for free.
...
Same secret, same hex encoding. Snippets:
...
- Keep your server NTP-synced; >5 min drift will reject valid events.
...
<AccordionGroup>
  ...
  <Accordion title="Will V1 ever go away?">
    Not without 60 days notice, emailed to your store's contact.
  </Accordion>
  <Accordion title="Same secret for V1 and V2?">
    Yes.
  </Accordion>
  <Accordion title="Can endpoints in the same store be on different modes?">
    Yes — `signatureVersion` is per-endpoint.
  </Accordion>
</AccordionGroup>
